...
export default function LicenseControlCenter() {
  ...
  const [ultraUpdate, setUltraUpdate] = useState<UltraUpdateSettings>({ version: "2.7.0", url: "", sha256: "", changelog: "", zxpUrl: "", zxpSha256: "" });
  ...
  async function saveUltraUpdate() {
    ...
    if (savingUltraUpdate || !/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(version)) { setError("Sürümü 2.7.0 biçiminde gir."); return; }
    ...
    <div className="license-os">
      ...
      <section className="admin-panel" style={{ display: "grid", gap: 14, padding: 20, marginBottom: 18 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "140px minmax(0,1fr)", gap: 10 }}>
          <label style={{ display: "grid", gap: 6 }}><span>Sürüm</span><input aria-label="Ultra sürümü" value={ultraUpdate.version} onChange={event => setUltraUpdate(current => ({ ...current, version: event.target.value }))} placeholder="2.7.0" /></label>
          <label style={{ display: "grid", gap: 6 }}><span>İlk kurulum ZXP linki</span><input aria-label="Ultra ZXP linki" value={ultraUpdate.zxpUrl} onChange={event => setUltraUpdate(current => ({ ...current, zxpUrl: event.target.value }))} placeholder="https://dromocob.tr/downloads/Dromocob-Ultra-2.7.0.zxp" /></label>
        </div>
        ...
        <label style={{ display: "grid", gap: 6 }}><span>Panel güncelleme ZIP linki</span><input aria-label="Ultra güncelleme ZIP linki" value={ultraUpdate.url} onChange={event => setUltraUpdate(current => ({ ...current, url: event.target.value }))} placeholder="https://dromocob.tr/downloads/Dromocob-Ultra-2.7.0-update.zip" /></label>
