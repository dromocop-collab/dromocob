...
export default function RegisterPage(){
  ...
  async function submit(e:FormEvent<HTMLFormElement>){e.preventDefault();setBusy(true);setErr("");const f=new FormData(e.currentTarget);const displayName=String(f.get("displayName")||"").trim(),email=String(f.get("email")||"").trim().toLowerCase(),password=String(f.get("password")||"");if(password.length<8||!/[A-ZÇĞİÖŞÜ]/.test(password)||!/\d/.test(password)){setBusy(false);setErr("Şifre en az 8 karakter, 1 büyük harf ve 1 rakam içermeli.");return;}try{const c=await createUserWithEmailAndPassword(auth,email,password);await updateProfile(c.user,{displayName});await setDoc(doc(db,"users",c.user.uid),{uid:c.user.uid,email,displayName,phone:"",company:"",city:"",bio:"",verified:false,status:"active",createdAt:serverTimestamp(),updatedAt:serverTimestamp()});const token=await c.user.getIdToken();await fetch("/api/auth/email-verification/send",{method:"POST",headers:{Authorization:`Bearer ${token}`}});router.push("/hesap-dogrulama");}catch(x){setErr(authMessage(typeof x==="object"&&x&&"code"in x?String(x.code):""));}finally{setBusy(false);}}
